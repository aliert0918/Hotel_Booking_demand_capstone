...
FEATURE_COLUMNS = ['country', 'market_segment', 'previous_cancellations', 'booking_changes', 'deposit_type', 'days_in_waiting_list', 'customer_type', 'reserved_room_type', 'required_car_parking_spaces', 'total_of_special_requests', 'is_repeat_canceler', 'is_high_risk', 'country_group', 'commitment_score', 'booking_stability'] [cite: 1]

# Nilai Unik Kategori (dari kolom_kategori_unique_values.csv)
CATEGORY_UNIQUE_VALUES = {
    'market_segment': ['Offline TA/TO', 'Online TA', 'Direct', 'Groups', 'Corporate', 'Complementary', 'Aviation', 'Other'], [cite: 2]
    'deposit_type': ['No Deposit', 'Non Refund', 'Refundable'], [cite: 2]
    'customer_type': ['Transient-Party', 'Transient', 'Contract', 'Group'], [cite: 2]
    'country_group': ['Top_International', 'PRT', 'Other'], [cite: 2]
    'reserved_room_type': ['A', 'E', 'D', 'F', 'B', 'G', 'C', 'H', 'L', 'P'] [cite: 2]
}

# Statistik Numerik (dari kolom_numerik_range.csv)
NUMERIC_RANGES = {
    'commitment_score': {'min': 0.0, 'max': 21.0, 'mean': 0.8571069603819416}, [cite: 3]
    'booking_stability': {'min': -39.1, 'max': 21.0, 'mean': -0.012159429480813196}, [cite: 3]
    'previous_cancellations': {'min': 0.0, 'max': 26.0, 'mean': 0.08679836789393704}, [cite: 3]
    'booking_changes': {'min': 0.0, 'max': 21.0, 'mean': 0.22089670108767184}, [cite: 3]
    'days_in_waiting_list': {'min': 0.0, 'max': 391.0, 'mean': 2.33056130568485}, [cite: 3]
...
def main():
    ...
    # Fitur 'country' memiliki terlalu banyak nilai unik untuk input UI yang praktis.
    # Asumsikan 'country' memiliki nilai default 'Other' atau 'PRT' (kode negara dengan jumlah data terbanyak)
    ...
    if st.button("Lakukan Prediksi Risiko", type="primary"):
        ...
        with st.spinner("Memproses prediksi..."):
            try:
                ...
                # Asumsi model.predict_proba mengembalikan array dengan probabilitas kelas 0 dan 1.
                prob_risk = model.predict_proba(input_df)[:, 1][0]
                
                # Prediksi Kelas (menggunakan threshold yang sudah dituning oleh TunedThresholdClassifierCV)
